...
const FROM    = process.env.RESEND_FROM_EMAIL || 'alerts@lawradar.com'
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://lawradar.com'
...
<body style="background:#070C18;color:#F1F5F9;font-family:sans-serif;padding:40px;max-width:580px;margin:0 auto">
  ...
  <p style="color:#374151;font-size:12px;margin:0">
    LawRadar &middot; General information only, not legal advice.
...
export async function sendMonthlyDigest({ to, userName, alerts }) {
  ...
  return getResend().emails.send({
    from:    FROM,
    to,
    subject: `LawRadar Monthly Digest &mdash; ${month}`,
    html: `<!DOCTYPE html>
<html>
